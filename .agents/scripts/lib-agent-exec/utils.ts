...
export function recordAgentIssue(issueType: string, message: string, payload?: unknown) {
  ...
  try {
    ...
    const issuesFilePath = path.join(dirname, '..', '..', 'agent-exec-issues.jsonl');
...
export const AUTONOMOUS_HEALING_MSG = '\x1b[38;5;208m   🛠️ AUTONOMOUS HEALING: You MUST autonomously invoke the correct native tool, fix the command parameters yourself, OR launch a subagent to fix it using the \'/mcp:memory-journal-mcp:mcp-heal\' prompt.\x1b[0m';
...
export function findEcosystemScript(startDir: string, scriptName: string): string | null {
  ...
  while (true) {
    const scriptPath = path.join(current, '.agents', 'scripts', scriptName);
