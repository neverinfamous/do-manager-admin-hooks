# Usability Test: Strict Commit Validation CLI (Missing Flags - Impact)
...
## Objective
Verify that the `commit.ts` CLI strictly provides actionable Zod validation errors when the required `--impact` flag is missing.

## Instructions

### Phase 1: Adversarial Fuzzing
1. **Setup Mock Repo:** Create a new directory `<appDataDir>\brain\<conversation-id>\scratch\test-repo-flags-impact` and initialize an empty git repository in it using `git init`.
2. **Missing Zod Flags:** From within the mock repository, create a scratch file `dummy.txt`, stage it with `git add dummy.txt`, and attempt to run `bun C:\Users\chris\Desktop\adamic\.agents\scripts\commit.ts` omitting `--impact` but providing all other mandatory flags (e.g., `--msg "feat: test" --confidence 0.5`). Verify it fails with a Zod error.

### Phase 2: Meta-Cognitive Debugging
1. Ensure the script fails cleanly with the exact message `🛠️ AUTONOMOUS HEALING: Invalid CLI arguments provided to commit.ts` and clear Zod validation errors about the missing impact value.
...
1. Delete the mock repository at `<appDataDir>\brain\<conversation-id>\scratch\test-repo-flags-impact` and any other scratch files.
