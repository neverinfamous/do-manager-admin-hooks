# Usability Test: Strict Commit Validation CLI (Validation Types - Impact)
...
## Objective
Verify that the `commit.ts` CLI strictly enforces actionable Zod validation errors for invalid data types passed to the `--impact` flag.

## Instructions

### Phase 1: Adversarial Fuzzing
1. **Setup Mock Repo:** Create a new directory `<appDataDir>\brain\<conversation-id>\scratch\test-repo-types-impact` and initialize an empty git repository in it using `git init`.
2. **Invalid Types:** From within the mock repository, create a scratch file `dummy.txt`, stage it with `git add dummy.txt`, and attempt to run `bun C:\Users\chris\Desktop\adamic\.agents\scripts\commit.ts` with an invalid data type for impact (e.g., passing a string `--impact "high"` instead of a float). Ensure you pass all other mandatory flags.
...
1. Delete the mock repository at `<appDataDir>\brain\<conversation-id>\scratch\test-repo-types-impact` and any other scratch files.
