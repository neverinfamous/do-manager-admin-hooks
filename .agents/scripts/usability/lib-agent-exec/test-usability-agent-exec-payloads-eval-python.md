# Usability Test: Agent-Exec Payloads (Eval - Python)
...
## Objective
Verify that `lib-agent-exec` correctly parses and handles the `.json` `eval` payload type for the `python` interpreter.

## Instructions

### Phase 1: Payload Construction
1. **Eval Payload (python):** Create a JSON payload (MUST be saved to `<appDataDir>\brain\<conversation-id>\scratch\payload.json`) for `eval` with interpreter `python`. Example: `{"type": "eval", "code": "print('hello')", "interpreter": "python", "cwd": "."}`. Execute and verify outputs.
