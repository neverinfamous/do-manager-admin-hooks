# Usability Test: Agent-Exec Payload Webhook - webhookMethod
...
## Objective
Verify that `lib-agent-exec` correctly parses and respects the `webhookMethod` option.

## Instructions

### Phase 1: Webhook Validation
1. **Webhook Method:** Create a `command` payload that includes `webhookMethod` (e.g. `"POST"`). Verify that upon completion, the proxy attempts to parse and use these fields without throwing validation errors.
