# Usability Test: Agent-Exec Payload Webhook - webhookTimeoutMs
...
## Objective
Verify that `lib-agent-exec` correctly parses and respects the `webhookTimeoutMs` option.

## Instructions

### Phase 1: Webhook Validation
1. **Webhook Timeout:** Set `webhookTimeoutMs` to `100` (ms). Verify that the proxy does not hang indefinitely and respects the schema bound.
