# Usability Test: Agent-Exec Payload Validation Output Capture - maxBuffer
...
## Objective
Verify that `lib-agent-exec` correctly parses and respects the `maxBuffer` option.

## Instructions

### Phase 1: Payload Options
1. **Max Buffer Limit:** Verify that the `maxBuffer` property is parsed correctly and restricts the buffer size appropriately according to the payload options.
