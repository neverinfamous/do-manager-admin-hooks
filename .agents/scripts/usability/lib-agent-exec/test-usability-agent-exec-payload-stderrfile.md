# Usability Test: Agent-Exec Payload Validation Output Capture - stderrFile
...
## Objective
Verify that `lib-agent-exec` correctly parses and respects the `stderrFile` option.

## Instructions

### Phase 1: Payload Options
1. **Output Redirects:** Pass a payload with `"stderrFile": "error.txt"`. Verify that standard error is correctly written to the specified file.
