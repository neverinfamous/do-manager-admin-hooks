...
## Objective
Verify that `lib-agent-exec` correctly intercepts the PowerShell alias `grep` when used in a command payload and throws an anti-hallucination error.

## Instructions

### Phase 1: Adversarial Execution
1. **FS Alias Hallucinations:** Attempt to run `grep pattern file.txt` as a raw command payload.
2. **Interception Check:** Verify the interceptor traps `grep` and throws a specific anti-hallucination error instructing the agent to NOT use this or its PowerShell cmdlet equivalent (`Select-String`). Instead, the error MUST instruct the agent to use the agent-native tools (`grep_search`).
