# Usability Test: Agent-Exec WSL2 Integration (Core - Script)
...
## Objective
Verify that `lib-agent-exec` correctly maps and executes scripts within the Windows Subsystem for Linux (WSL2) when explicitly requested via the payload target.

## Instructions

### Phase 1: WSL2 Execution
1. **Target Script:** Create a JSON payload (MUST be saved to `<appDataDir>\brain\<conversation-id>\scratch\payload.json`) with `"type": "script"`, `"target": "wsl2"`, `"scriptPath"`, and optionally `"interpreter"`. Execute via `bun .agents/scripts/agent-exec.ts <path>`. Attempt to execute a basic Linux bash script. Verify that the script executes inside WSL2.
