# Usability Test: Agent-Exec Payloads (Eval - Node)
...
## Objective
Verify that `lib-agent-exec` correctly parses and handles the `.json` `eval` payload type for the `node` interpreter.

## Instructions

### Phase 1: Payload Construction
1. **Eval Payload (node):** Create a JSON payload (MUST be saved to `<appDataDir>\brain\<conversation-id>\scratch\payload.json`) for `eval` with interpreter `node`. Example: `{"type": "eval", "code": "console.log('hello')", "interpreter": "node", "cwd": "."}`. Execute and verify outputs.
