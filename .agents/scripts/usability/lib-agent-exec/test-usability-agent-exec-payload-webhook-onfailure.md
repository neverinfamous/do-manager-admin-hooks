# Usability Test: Agent-Exec Payload Webhook - onFailure
...
## Objective
Verify that `lib-agent-exec` correctly parses and respects the `onFailure` option.

## Instructions

### Phase 1: Webhook Validation
1. **Webhook OnFailure:** Create a `command` payload that includes `onFailure` URLs. Verify that upon completion, the proxy attempts to parse and use these fields without throwing validation errors.
