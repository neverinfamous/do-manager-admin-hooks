# Usability Test: Agent-Exec Payload Validation Output Capture - stdoutFile
...
## Objective
Verify that `lib-agent-exec` correctly parses and respects the `stdoutFile` option.

## Instructions

### Phase 1: Payload Options
1. **Output Redirects:** Pass a payload with `"stdoutFile": "output.txt"`. Verify that standard output is correctly written to the specified file.
