# Usability Test: Agent-Exec Payload Timeout - stallTimeoutMs
...
## Objective
Verify that `lib-agent-exec` correctly parses and respects the `stallTimeoutMs` option.

## Instructions

### Phase 1: Adversarial Execution
1. **Stall Timeout:** Test payload with `stallTimeoutMs` set to 100ms. Run a command that outputs nothing for 1 second but does not exit. Verify the process is killed due to stalling.
