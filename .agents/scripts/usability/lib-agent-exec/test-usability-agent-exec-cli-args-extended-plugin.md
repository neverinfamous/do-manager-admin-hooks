# Usability Test: Agent-Exec CLI Args - --plugin
...
## Objective
Verify that `lib-agent-exec` correctly parses and respects the `--plugin` option.

## Instructions

### Phase 1: CLI Arguments Validation
1. **Plugin Flag:** Run `bun agent-exec.ts --plugin "./my-plugin.ts" <payload.json>` (with a mock plugin). Verify the CLI correctly resolves and executes the plugin instead of standard evaluation.
