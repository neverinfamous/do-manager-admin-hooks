# Usability Test: Agent-Exec Payloads (Eval - Bun)
...
## Objective
Verify that `lib-agent-exec` correctly parses and handles the `.json` `eval` payload type for the `bun` interpreter explicitly.

## Instructions

### Phase 1: Payload Construction
1. **Eval Payload (bun explicit):** Create a JSON payload (MUST be saved to `<appDataDir>\brain\<conversation-id>\scratch\payload.json`) for `eval` with interpreter `bun`. Example: `{"type": "eval", "code": "console.log('hello from bun')", "interpreter": "bun", "cwd": "."}`. Execute via `bun agent-exec.ts <payload.json>` and verify outputs.
