# Usability Test: Agent-Exec Payload Webhook - webhookPayloadTemplate
...
## Objective
Verify that `lib-agent-exec` correctly parses and respects the `webhookPayloadTemplate` option.

## Instructions

### Phase 1: Webhook Validation
1. **Webhook Payload Template:** Create a `command` payload that includes a `webhookPayloadTemplate` string. Verify that upon completion, the proxy attempts to parse and use these fields without throwing validation errors.
