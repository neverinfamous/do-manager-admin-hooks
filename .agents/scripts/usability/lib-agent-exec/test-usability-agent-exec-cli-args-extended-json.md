# Usability Test: Agent-Exec CLI Args - --json
...
## Objective
Verify that `lib-agent-exec` correctly parses and respects the `--json` option.

## Instructions

### Phase 1: CLI Arguments Validation
1. **JSON Output Explicit:** Run `bun agent-exec.ts --json <payload.json>`. Verify it forces JSON formatted output to stdout instead of human-readable logs.
