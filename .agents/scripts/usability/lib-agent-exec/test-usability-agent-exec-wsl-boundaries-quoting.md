# Usability Test: Agent-Exec WSL2 Integration (Boundaries - Quoting)
...
## Objective
Verify that `lib-agent-exec` maintains strict boundary fidelity regarding quoting and escaping when crossing into WSL2.

## Instructions

### Phase 1: Boundary Fidelity
1. **Quoting & Escaping:** Construct a JSON payload with `"type": "command"`, `"target": "wsl2"`, and `"command"`. Execute via `bun .agents/scripts/agent-exec.ts <path>`. Test complex JSON payloads and quoted strings (e.g., `awk '{print $1}'`) across the boundary. Verify that strings do not suffer from stripping, double-escaping, or corruption when passed to `wsl bash -c`.

### Phase 2: Meta-Cognitive Debugging
If the boundary corrupts payload strings:
