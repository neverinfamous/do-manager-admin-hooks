...
## Objective
Verify that `lib-agent-exec` correctly intercepts the PowerShell alias `cat` when used in a command payload and throws an anti-hallucination error.

## Instructions

### Phase 1: Adversarial Execution
1. **FS Alias Hallucinations:** Attempt to run `cat file.txt` as a raw command payload.
2. **Interception Check:** Verify the interceptor traps `cat` and throws a specific anti-hallucination error instructing the agent to NOT use this or its PowerShell cmdlet equivalent (`Get-Content`). Instead, the error MUST instruct the agent to use the agent-native tools (`view_file`).
