...
## Objective
Verify that `lib-agent-exec` gracefully handles `exit code 1` for `pnpm outdated` instead of treating it as a fatal agent error.

## Instructions

### Phase 1: Adversarial Execution
1. **PNPM Outdated:** Run `pnpm outdated`. Exit code 1 indicates outdated packages, not a crash. Verify graceful handling.
2. **True Errors:** Run a command that legitimately fails (e.g. `pnpm add NON_EXISTENT_PACKAGE_NAME`). Verify it still reports as a standard error properly (not gracefully swallowed).
