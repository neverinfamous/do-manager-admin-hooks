# Usability Test: Agent-Exec Payload Webhook - webhookHeaders
...
## Objective
Verify that `lib-agent-exec` correctly parses and respects the `webhookHeaders` option.

## Instructions

### Phase 1: Webhook Validation
1. **Webhook Headers:** Create a `command` payload that includes `webhookHeaders` (e.g. `{"Authorization": "Bearer token"}`). Verify that upon completion, the proxy attempts to parse and use these fields without throwing validation errors.
