# Usability Test: Git History Analytics (CLI Search Filter)

> [!IMPORTANT]
> **STRICT PROTOCOL ENFORCED:** You must parse git history non-destructively and never mutate the commit log.

## Objective
Verify that `lib-git-history`'s CLI correctly implements the `--search` message filter.

## Instructions

### Phase 1: Validating Extraction
1. **Extraction:** Create test commits that contain specific keywords in their commit messages (e.g., "feat(test): keyword_search_1").
2. Extract history using `bun .\.agents\scripts\get-git-history-json.ts --search "keyword_search_1"` and redirect to a scratch file.
