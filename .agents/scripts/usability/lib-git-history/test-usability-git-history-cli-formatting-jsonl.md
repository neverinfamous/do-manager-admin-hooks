# Usability Test: Git History Analytics (CLI Formatting - JSONL)

> [!IMPORTANT]
> **STRICT PROTOCOL ENFORCED:** You must parse git history non-destructively and never mutate the commit log.

## Objective
Verify that `lib-git-history`'s CLI correctly implements the `--jsonl` formatting flag.

## Instructions

### Phase 1: Validating Extraction
1. **Extraction:** Extract history using the following combination (redirecting to a scratch file):
   - `bun .\.agents\scripts\get-git-history-json.ts --limit 5 --jsonl`

### Phase 2: Meta-Cognitive Debugging
1. Verify that `--jsonl` outputs NDJSON (newline-delimited JSON) instead of a JSON array.
