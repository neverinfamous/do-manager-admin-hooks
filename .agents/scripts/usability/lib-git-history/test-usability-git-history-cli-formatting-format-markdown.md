# Usability Test: Git History Analytics (CLI Formatting - Markdown Format)

> [!IMPORTANT]
> **STRICT PROTOCOL ENFORCED:** You must parse git history non-destructively and never mutate the commit log.

## Objective
Verify that `lib-git-history`'s CLI correctly implements the Markdown formatting output `--format "markdown"`.

## Instructions

### Phase 1: Validating Extraction
1. **Extraction:** Extract history using the following (redirecting to a scratch file):
   - `bun .\.agents\scripts\get-git-history-json.ts --limit 5 --format "markdown"`

### Phase 2: Meta-Cognitive Debugging
1. Verify that `--format "markdown"` outputs markdown text.
...
1. **Crucial Cleanup:** Delete scratch markdown files.
