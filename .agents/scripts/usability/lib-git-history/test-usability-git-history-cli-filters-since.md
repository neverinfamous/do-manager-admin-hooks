# Usability Test: Git History Analytics (CLI Time Filter - Since)

> [!IMPORTANT]
> **STRICT PROTOCOL ENFORCED:** You must parse git history non-destructively and never mutate the commit log.

## Objective
Verify that `lib-git-history`'s CLI correctly implements the `--since` filter.

## Instructions

### Phase 1: Validating Extraction
1. **Extraction:** Use `bun .\.agents\scripts\get-git-history-json.ts --since "1 week ago"` to extract history.
2. Redirect output to `<appDataDir>\brain\<conversation-id>\scratch\time-filter-since-test.json`.
...
1. **Crucial Cleanup:** Delete `time-filter-since-test.json` and any scratch files.
