# Usability Test: Git History Analytics (CLI Limit)

> [!IMPORTANT]
> **STRICT PROTOCOL ENFORCED:** You must parse git history non-destructively and never mutate the commit log.

## Objective
Verify that `lib-git-history`'s CLI correctly implements the size limiting flag `--limit` as defined in `cliArgsSchema`.

## Instructions

### Phase 1: Validating Extraction
1. **Extraction:** Check the total number of commits in the repository.
2. Extract history using the following (redirecting to a scratch file):
   - `bun .\.agents\scripts\get-git-history-json.ts --limit 3`

### Phase 2: Meta-Cognitive Debugging
1. Verify that `--limit` correctly restrains the output to exactly 3 commits.
2. If the flag fails to limit the output, formulate a hypothesis in a scratch file.
