# Usability Test: Git History Analytics (CLI Formatting - No Body)

> [!IMPORTANT]
> **STRICT PROTOCOL ENFORCED:** You must parse git history non-destructively and never mutate the commit log.

## Objective
Verify that `lib-git-history`'s CLI correctly implements the `--no-body` formatting flag.

## Instructions

### Phase 1: Validating Extraction
1. **Extraction:** Extract history using the following combination (redirecting to a scratch file):
   - `bun .\.agents\scripts\get-git-history-json.ts --limit 5 --no-body`

### Phase 2: Meta-Cognitive Debugging
1. Verify that `--no-body` explicitly strips out the commit body and patches, leaving only the header metadata.
