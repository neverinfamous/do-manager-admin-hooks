# Usability Test: Git History Analytics (Traversal - Include Merges)

> [!IMPORTANT]
> **STRICT PROTOCOL ENFORCED:** You must parse git history non-destructively and never mutate the commit log.

## Objective
Verify that `lib-git-history`'s CLI correctly implements the `--include-merges` flag.

## Instructions

### Phase 1: Validating Extraction
1. **Extraction:** Extract history using:
   - `bun .\.agents\scripts\get-git-history-json.ts --limit 5 --include-merges`
2. Redirect to a scratch file.

### Phase 2: Meta-Cognitive Debugging
1. Verify that `--include-merges` returns merge commits (which are often filtered out by default).
